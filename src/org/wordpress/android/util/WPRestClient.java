...
public class WPRestClient {
     ...
     * 
     * https://developer.wordpress.com/docs/api/1/post/sites/%24site/posts/%24
     * post_ID/replies/new/
     */
    public void replyToComment(Note.Reply reply, Listener listener,
     ...
     * post_ID/replies/new/
     */
    public void replyToComment(String siteId, String commentId, String content,
            Listener listener, ErrorListener errorListener) {
        Map<String, String> params = new HashMap<String, String>();
        params.put(COMMENT_REPLY_CONTENT_FIELD, content);
        String path = String.format("sites/%s/comments/%s/replies/new", siteId,
                commentId);
     ...
     * /
     */
    public void followSite(String siteId, Listener listener,
            ErrorListener errorListener) {
     ...
     * 
     * https://developer.wordpress.com/docs/api/1/post/sites/%24site/follows/
     * mine/delete/
     */
    public void unfollowSite(String siteId, Listener listener,
            ErrorListener errorListener) {
     ...
     */
    public void getNotification(String noteId, Listener listener,
            ErrorListener errorListener) {
     ...
     */
    public void markNoteAsRead(Note note, Listener listener,
            ErrorListener errorListener) {
        String path = "notifications/read";
        Map<String, String> params = new HashMap<String, String>();
        params.put(String.format("counts[%s]", note.getId()),
                note.getUnreadCount());
        post(path, params, null, listener, errorListener);
    }
     ...
     * https://developer.wordpress.com/docs/api/1/get/notifications/
     */
    public void getNotifications(Map<String, String> params, Listener listener,
            ErrorListener errorListener) {
     ...
     */
    public void markNotificationsSeen(String timestamp, Listener listener,
            ErrorListener errorListener) {
        Map<String, String> params = new HashMap<String, String>();
     ...
     * Moderate a comment.
     * 
     * http://developer.wordpress.com/docs/api/1/sites/%24site/comments/%24
     * comment_ID/
     */
    ...
    public void get(String path, Map<String, String> params,
            RetryPolicy retryPolicy, Listener listener,
            ErrorListener errorListener) {
        // turn params into querystring

        RestRequest request = mRestClient.makeRequest(Method.GET,
                RestClient.getAbsoluteURL(path, params), null, listener,
                errorListener);
        if (retryPolicy == null) {
            retryPolicy = new DefaultRetryPolicy(REST_TIMEOUT_MS,
                    REST_MAX_RETRIES_GET, REST_BACKOFF_MULT);
        }
    ...
    public void post(final String path, Map<String, String> params,
        ...
        final RestRequest request = mRestClient.makeRequest(Method.POST,
                RestClient.getAbsoluteURL(path), params, listener,
                errorListener);
        if (retryPolicy == null) {
            retryPolicy = new DefaultRetryPolicy(REST_TIMEOUT_MS,
                    REST_MAX_RETRIES_POST, REST_BACKOFF_MULT); //Do not retry on failure
        }
        request.setRetryPolicy(retryPolicy);
        Request authCheck = new Request(request, errorListener);
        authCheck.send();
    ...
    /**
     * Interface that provides a method that should perform the necessary task
     * to make sure the provided Request will be authenticated.
     * 
     * The Authenticator must call Request.send() when it has completed its
     * operations. For convenience the Request class provides
     * Request.setAccessToken so the Authenticator can easily update the access
     * token.
     */
    ...
    /**
     * Encapsulates the behaviour for asking the Authenticator for an access
     * token. This allows the request maker to disregard the authentication
     * state when making requests.
     */
    public class Request {
        ...
        /**
         * Attempt to send the request, checks to see if we have an access token
         * and if not asks the Authenticator to authenticate the request.
         * 
        ...
        /**
         * If an access token cannot be obtained the request can be aborted and
         * the handler's onFailure method is called
         */
        ...
        /**
         * Implement this method to perform the request that should be
         * authenticated
         */
